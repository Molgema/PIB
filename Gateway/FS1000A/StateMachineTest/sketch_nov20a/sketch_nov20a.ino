...
void setup() {
  Serial.begin(115200);
  pinMode(SW1, INPUT); //Instellen van input prog knop  
...
int ConvertString(String string) {
  ...
  else if (!digitalRead(SW1)) convertedString = 7;
...
void loop() {
  ...
  static int conString = ;

  if ((Serial.available() > 0) || !(digitalRead(SW1))) {
    if (Serial.available() > 0) {
      static String string = Serial.readStringUntil('\n');
      Serial.println("Het ingevoerde commando is " + string);
      static int conString = ConvertString(string);
    }
    if (!digitalRead(SW1)) {
      static String string = "Prog";
      Serial.println("Het ingevoerde commando is " + string);
      static int conString = ConvertString(string);
    }
